...
from bot.commands import unknown, error, start, keyboard, DATE, new_element, NAME, add_name, cancel, calendar_set, \
    ...
    NEW_SHEET_DATE, new_sheet_date_choose, RECURRENT_NAME, RECURRENT_IMPORT, RECURRENT_DELETE, \
    select_delete_recurrent, select_recurrent_date, new_recurrent_name, RECURRENT_DATE, new_recurrent_import, \
    new_recurrent_insert, SELECT_TYPE, new_recurrent_type, RECURRENT_CONFIRM_DELETE, confirm_delete_recurrent, \
    help_command
...
def set_handlers(dispatcher: Dispatcher):
    ...
    recurrent_entry_points = [
        ...
        CommandHandler(COMMANDS['new_recurrent'].command,
                       lambda update, context: new_recurrent_type(update, context, keyboard[8]),
                       Filters.user(user_id=int(USER_ID))),
    ]
    ...
    recurrent_delete_entry_points = [
        MessageHandler(message_filter(keyboard[9]), new_recurrent_type),
        CommandHandler(COMMANDS['delete_recurrent'].command,
                       lambda update, context: new_recurrent_type(update, context, keyboard[9]),
                       Filters.user(user_id=int(USER_ID))),
    ]
    recurrent_delete_handler = ConversationHandler(
        entry_points=recurrent_delete_entry_points,
        states={
            RECURRENT_DELETE: [CallbackQueryHandler(select_delete_recurrent)],
            RECURRENT_CONFIRM_DELETE: [MessageHandler(text_filter, confirm_delete_recurrent)]
        },
    ...
    # Help button
    dispatcher.add_handler(MessageHandler(message_filter(keyboard[-1]), help_command))
    dispatcher.add_handler(CommandHandler(COMMANDS['help'].command, help_command, Filters.user(user_id=int(USER_ID))))
